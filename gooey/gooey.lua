...
function M.list(root_id, node_ids, action_id, action, fn)
	...
	if #node_ids == 0 then return list end
	
	local over_node
	local over_index
	for i=1,#node_ids do
		local node = gui.get_node(node_ids[i])
		list.nodes[i] = node
	...
	list.over = over_node ~= nil
	list.over_node = over_node
	list.over_index = over_index

	local first_node = list.nodes[1]
	local last_node = list.nodes[#list.nodes]
	local total_height = math.abs(gui.get_position(last_node).y) + gui.get_size(last_node).y / 2
	local list_height = gui.get_size(list.root).y

	if action_id == TOUCH then
		if not list.enabled then
			list.pressed_node = nil
			list.pressed_index = nil
			list.selected_node = nil
			list.selected_index = nil
		elseif action.pressed and list.over then
			list.pressed_node = over_node
			list.pressed_index = over_index
			list.root_pos = gui.get_position(root)
			list.action_pos = vmath.vector3(action.x, action.y, 0)
		elseif action.released then
			list.pressed_node = nil
			list.pressed_index = nil
			if list.over and not list.scrolling then
				list.selected_node = over_node
				list.selected_index = over_index
				fn(list)
			end
		end
	end
	
	if list.pressed_node then
